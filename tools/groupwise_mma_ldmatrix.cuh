#ifndef LAB45_GROUPWISE_MMA_LDMATRIX_CUH
#define LAB45_GROUPWISE_MMA_LDMATRIX_CUH

#include <cstddef>
#include <cstdint>

namespace lab45_mma_ldmatrix {
...
__device__ __forceinline__ int swizzled_offset(int row, int k) {
    const int linear = row * kCtaK + k;
    return linear ^ ((linear & 0x180) >> 3);
}
...
__device__ __forceinline__ void copy_async_wait_one() {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 800
    asm volatile("cp.async.wait_group 1;\n" ::);
#endif
}

__device__ __forceinline__ void copy_async_wait_all() {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 800
...
__device__ __forceinline__ void ldmatrix_x2(
    unsigned shared_address, uint32_t& r0, uint32_t& r1) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 750
    asm volatile(
        "ldmatrix.sync.aligned.x2.m8n8.shared.b16 {%0, %1}, [%2];\n"
        : "=r"(r0), "=r"(r1) : "r"(shared_address));
#endif
}
...
__device__ __forceinline__ void stage_tile(
    const int8_t* aq, const int8_t* bq,
    int8_t* shared_a, int8_t* shared_b,
    size_t extended_k, int K, int cta_m, int cta_n,
    int a_slot, int j_low, int k_offset, int stage) {
    const int thread = threadIdx.x;
    int8_t* stage_a = shared_a
        + static_cast<size_t>(stage) * kCtaM * kCtaK;
    int8_t* stage_b = shared_b
        + static_cast<size_t>(stage) * kCtaN * kCtaK;
    for (int vector = thread;
         vector < kCtaM * (kCtaK / 16);
         vector += kThreads) {
        const int row = vector / (kCtaK / 16);
        const int segment = vector % (kCtaK / 16);
        const int column = segment * 16;
        const int8_t* source = aq
            + static_cast<size_t>(cta_m + row) * extended_k
            + static_cast<size_t>(a_slot) * K + k_offset + column;
        copy_async_16(stage_a + swizzled_offset(row, column), source);
    }
    for (int vector = thread;
        ...
        const int segment = vector % (kCtaK / 16);
        const int column = segment * 16;
        const int8_t* source = bq
            + static_cast<size_t>(cta_n + row) * extended_k
            + static_cast<size_t>(j_low) * K + k_offset + column;
        copy_async_16(stage_b + swizzled_offset(row, column), source);
    }
    copy_async_commit();
}
...
void groupwise_mma_kernel(const int8_t* __restrict__ aq,
                          ...
                          int M, int N, int K) {
    __shared__ __align__(16) int8_t shared_a[kStages * kCtaM * kCtaK];
    __shared__ __align__(16) int8_t shared_b[kStages * kCtaN * kCtaK];
...
#pragma unroll
        ...
        int stage = 0;
        stage_tile<Splits>(aq, bq, shared_a, shared_b, extended_k, K,
                           cta_m, cta_n, a_slot, j_low, 0, stage);
        for (int k_base = 0; k_base < total_k; k_base += kCtaK) {
            const int next_k = k_base + kCtaK;
            if (next_k < total_k) {
                stage_tile<Splits>(
                    aq, bq, shared_a, shared_b, extended_k, K,
                    cta_m, cta_n, a_slot, j_low, next_k, stage ^ 1);
                copy_async_wait_one();
            } else {
                copy_async_wait_all();
            }
            ...
            const int8_t* stage_a = shared_a
                + static_cast<size_t>(stage) * kCtaM * kCtaK;
            const int8_t* stage_b = shared_b
                + static_cast<size_t>(stage) * kCtaN * kCtaK;
            const unsigned stage_a_address = static_cast<unsigned>(
                __cvta_generic_to_shared(stage_a));
            const unsigned stage_b_address = static_cast<unsigned>(
                __cvta_generic_to_shared(stage_b));
#pragma unroll
...
#pragma unroll
                for (int m = 0; m < kWarpTilesM; ++m) {
                    const int row = warp_m * kWarpM + m * kMmaM
                                  + (lane & 7) + 8 * (quad & 1);
                    const int column = k_inner + 16 * (quad >> 1);
                    ldmatrix_x4(stage_a_address + swizzled_offset(row, column),
                                fragment_a[m][0], fragment_a[m][1],
                                fragment_a[m][2], fragment_a[m][3]);
                }
#pragma unroll
                for (int n = 0; n < kWarpTilesN; ++n) {
                    const int row = warp_n * kWarpN + n * kMmaN
                                  + (lane & 7);
                    const int column = k_inner + 16 * (quad & 1);
                    ldmatrix_x2(stage_b_address + swizzled_offset(row, column),
